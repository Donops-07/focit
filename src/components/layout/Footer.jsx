import { Link } from "react-router-dom";
import {
  GraduationCap,
  Mail,
  Phone,
  MapPin,
  ExternalLink,
  ArrowUp,
} from "lucide-react";
import { FOOTER_LINKS, SOCIAL_LINKS, FACULTY_INFO } from "../../data/navigation";

/* Inline SVG social icons — lucide-react does not export brand icons */
const FacebookIcon = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
  </svg>
);
const TwitterIcon = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
  </svg>
);
const LinkedinIcon = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
);
const InstagramIcon = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
  </svg>
);

const ICON_MAP = {
  facebook: FacebookIcon,
  twitter: TwitterIcon,
  linkedin: LinkedinIcon,
  instagram: InstagramIcon,
};

/**
 * Footer — Responsive multi-column footer with brand info,
 * quick links, and social links. Mirrors UNIOSUN institutional style.
 */
export default function Footer() {
  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const currentYear = new Date().getFullYear();

  return (
    <footer className="site-footer">
      {/* Back to Top */}
      <button
        className="back-to-top"
        onClick={scrollToTop}
        aria-label="Scroll to top"
      >
        <ArrowUp size={20} />
      </button>

      <div className="footer__inner">
        {/* Brand Column */}
        <div className="footer__brand">
          <Link to="/" className="footer__logo">
            <div className="footer__logo-icon overflow-hidden flex items-center justify-center bg-white rounded-full p-1">
              <img src="/faculty-logo.jpg" alt="FOCITSA Logo" className="w-full h-full object-contain" />
            </div>
            <div className="footer__logo-text">
              <span className="footer__logo-name">
                {FACULTY_INFO.name}
              </span>
              <span className="footer__logo-uni">
                {FACULTY_INFO.university}
              </span>
            </div>
          </Link>

          <p className="footer__description">
            Empowering the next generation of computing professionals through
            innovative education, impactful research, and community engagement.
          </p>

          {/* Contact Info */}
          <div className="footer__contact">
            <a
              href={`mailto:${FACULTY_INFO.email}`}
              className="footer__contact-link"
            >
              <Mail size={16} />
              <span>{FACULTY_INFO.email}</span>
            </a>
            <a
              href={`tel:${FACULTY_INFO.phone}`}
              className="footer__contact-link"
            >
              <Phone size={16} />
              <span>{FACULTY_INFO.phone}</span>
            </a>
            <span className="footer__contact-link">
              <MapPin size={16} />
              <span>{FACULTY_INFO.address}</span>
            </span>
          </div>

          {/* Social Links */}
          <div className="footer__socials">
            {SOCIAL_LINKS.map((social) => {
              const Icon = ICON_MAP[social.icon];
              return (
                <a
                  key={social.label}
                  href={social.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="footer__social-link"
                  aria-label={social.label}
                >
                  {Icon && <Icon size={18} />}
                </a>
              );
            })}
          </div>
        </div>

        {/* Link Columns */}
        {FOOTER_LINKS.map((group) => (
          <div key={group.heading} className="footer__column">
            <h3 className="footer__column-heading">{group.heading}</h3>
            <ul className="footer__column-list">
              {group.links.map((link) => (
                <li key={link.path}>
                  <Link to={link.path} className="footer__column-link">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        ))}

        {/* External Resources Column */}
        <div className="footer__column">
          <h3 className="footer__column-heading">Resources</h3>
          <ul className="footer__column-list">
            <li>
              <a
                href="https://uniosun.edu.ng"
                target="_blank"
                rel="noopener noreferrer"
                className="footer__column-link footer__column-link--external"
              >
                UNIOSUN Portal
                <ExternalLink size={12} />
              </a>
            </li>
            <li>
              <a
                href="https://uniosun.edu.ng"
                target="_blank"
                rel="noopener noreferrer"
                className="footer__column-link footer__column-link--external"
              >
                Student Portal
                <ExternalLink size={12} />
              </a>
            </li>
            <li>
              <a
                href="https://portal.uniosun.edu.ng"
                target="_blank"
                rel="noopener noreferrer"
                className="footer__column-link footer__column-link--external"
              >
                E-Library
                <ExternalLink size={12} />
              </a>
            </li>
          </ul>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="footer__bottom">
        <div className="footer__bottom-inner">
          <p className="footer__copyright">
            &copy; {currentYear} {FACULTY_INFO.name},{" "}
            {FACULTY_INFO.university}. All rights reserved.
          </p>
          <p className="footer__credit">
            Built by FOCITSA Students
          </p>
        </div>
      </div>
    </footer>
  );
}
